...
export default function Material() {
    ...
    const typeColors = {
        Tugas: "bg-blue-50 text-blue-600 border-blue-100",
        Ujian: "bg-red-50 text-red-600 border-red-100",
        Latihan: "bg-green-50 text-green-600 border-green-100",
    };
    const formatColors = {
        PDF: "bg-red-50 text-red-600",
        PPTX: "bg-amber-50 text-amber-600",
        MP4: "bg-blue-50 text-blue-600",
    ...
    return (
        <DashboardLayout headerTitle="Tugas & Materi">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-5 flex flex-col sm:flex-row sm:items-center gap-4">
                    <div className="flex items-center gap-3 flex-1">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">
                                Semua tugas dan materi pembelajaran di sekolah
                                Anda.
                            </p>
                        </div>
                    </div>
                    <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-100 shrink-0">
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-xs font-medium transition ${tab === "tasks" ? "bg-primary-500 text-white" : "text-gray-500"}`}
                        >
                            Tugas
                        </button>
                        <button
                            onClick={() => setTab("materials")}
                            className={`px-4 py-2 rounded-lg text-xs font-medium transition ${tab === "materials" ? "bg-primary-500 text-white" : "text-gray-500"}`}
                        >
                            Materi
                        </button>
                    </div>
                        ...
                        {dummyTasks.map((t) => (
                            ...
                            >
                                <div className="flex items-start justify-between mb-2">
                                    ...
                                    <span
                                        className={`text-[10px] font-semibold px-2.5 py-1 rounded-full border ${typeColors[t.type] || ""}`}
                                    >
                                        {t.type}
                                    </span>
                                </div>
                                <div className="flex items-center gap-4 text-xs text-gray-500 mt-3">
                                    ...
                                    <span className="font-medium text-primary-500">
                                        {t.submissions}/{t.total} dikumpulkan
                                    </span>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="space-y-3">
                        {dummyMaterials.map((m) => (
                            <div
                                key={m.id}
                                onClick={() =>
                                    setDetailItem({ ...m, _type: "material" })
                                }
                                className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-sm transition cursor-pointer group"
                            >
                                <div className="flex items-start justify-between mb-2">
                                    <div className="flex items-center gap-3">
                                        <div
                                            className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-bold ${formatColors[m.format] || "bg-gray-50 text-gray-600"}`}
                                        >
                                            {m.format}
                                        </div>
                                        <div>
            ...
            <Modal
                ...
                title={`Detail ${detailItem?._type === "task" ? "Tugas" : "Materi"}`}
                maxWidth="max-w-lg"
            >
                {detailItem && (
                    <>
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {[
                                    { l: "Mapel", v: detailItem.subject },
                                    { l: "Guru", v: detailItem.teacher },
                                    { l: "Kelas", v: detailItem.class },
                                    detailItem._type === "task"
                                        ? {
                                              l: "Pengumpulan",
                                              v: `${detailItem.submissions}/${detailItem.total}`,
                                          }
                                        : {
                                              l: "Ukuran File",
                                              v: detailItem.size,
